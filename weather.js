...
const getWeatherDetails = () => {
  ...
  // console.log(entered);
  const apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${entered}&appid=987d7a12fd4624a65be1b88d3294830c&units=metric&lang=en`;
  ...
  $.ajax({
    ...
    success: (response) => {
      ...
      if (arrayListing.length > 0) {
        let filtered = arrayListing.filter(
          (theLi) => $(theLi).find(".city-name span").text() == name
        );
      ...
      if (!foundDuplicate) {
        ...
        let liContent = `
                    ...
                    <figure>
                        <img class="city-icon" src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/162656/${
                          weather[0].icon
                        }.svg">
